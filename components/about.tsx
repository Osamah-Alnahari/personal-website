...
export default function About() {
  ...
  const highlights = [
    ...
    },
    {
      ...
      description: "Designing scalable infrastructure with AWS, Docker, Kubernetes, and serverless technologies",
    },
    {
      icon: <Cpu className="w-8 h-8 text-secondary" />,
      title: "Full-Stack Engineering",
      description: "Building end-to-end solutions with React, Next.js, Node.js, and modern development practices",
    },
  ]
  ...
  return (
    <section ref={sectionRef} id="about" className="py-20 bg-muted/30 relative overflow-hidden">
      <div className="absolute inset-0 pointer-events-none">
        <Bot className="floating-icon absolute top-20 left-10 text-primary" size={24} />
        <Database className="floating-icon absolute top-40 right-20 text-accent" size={28} />
        <Cpu className="floating-icon absolute bottom-40 left-20 text-secondary" size={32} />
        <Cloud className="floating-icon absolute bottom-20 right-10 text-primary" size={26} />
        <Brain className="floating-icon absolute top-60 left-1/3 text-accent" size={30} />
        <Zap className="floating-icon absolute bottom-60 right-1/3 text-secondary" size={22} />
      </div>
      ...
      <div className="max-w-7xl 3xl:max-w-8xl 4xl:max-w-9xl 5xl:max-w-10xl mx-auto px-4 sm:px-6 lg:px-8 3xl:px-12 4xl:px-16 5xl:px-20 relative z-10">
        <div className="text-center mb-12 md:mb-16 3xl:mb-20 4xl:mb-24">
          <h2 className="text-3xl xs:text-4xl md:text-4xl 3xl:text-5xl 4xl:text-5xl 5xl:text-6xl font-bold text-foreground mb-6 gsap-fade-up">
            About Me
          </h2>
          <p className="text-base xs:text-lg 3xl:text-xl 4xl:text-xl text-muted-foreground max-w-3xl 3xl:max-w-4xl 4xl:max-w-5xl mx-auto gsap-fade-up text-pretty">
            AI Research Engineer and Software Developer specializing in intelligent systems, cloud architecture, and
            scalable applications that bridge cutting-edge research with real-world solutions.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 md:gap-12 3xl:gap-16 4xl:gap-20 5xl:gap-24 items-center">
          ...
          <div ref={cardsRef} className="grid gap-4 md:gap-6 3xl:gap-8 4xl:gap-10">
            {highlights.map((highlight, index) => (
              <Card
                key={index}
                className="highlight-card hover:shadow-lg hover:shadow-primary/10 transition-all duration-300 border-border/50 bg-card/80 backdrop-blur-sm"
              >
                <CardContent className="p-4 sm:p-6 3xl:p-8 4xl:p-10">
                  <div className="flex items-start space-x-3 sm:space-x-4 3xl:space-x-6">
                    <div className="flex-shrink-0">
                      <div className="w-8 h-8 sm:w-10 sm:h-10 3xl:w-12 3xl:h-12 4xl:w-14 4xl:h-14 flex items-center justify-center bg-primary/10 rounded-lg">
