...
export default function Experience() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo(
        ".timeline-card",
        { opacity: 0, x: (index) => (index % 2 === 0 ? -30 : 30), scale: 0.95 },
        {
          opacity: 1,
          x: 0,
          scale: 1,
  ...
  return (
    ...
    >
      <div className="absolute inset-0 pointer-events-none">
        <Brain
          className="floating-bg-icon absolute top-20 left-10 text-primary"
          size={32}
        />
        <Code
          className="floating-bg-icon absolute top-40 right-20 text-accent"
          size={28}
        />
        <Zap
          className="floating-bg-icon absolute bottom-40 left-20 text-secondary"
          size={30}
        />
        <GraduationCap
          className="floating-bg-icon absolute bottom-20 right-10 text-primary"
          size={26}
        />
        <Brain
          className="floating-bg-icon absolute top-60 right-1/3 text-accent"
          size={24}
        />
      </div>
      ...
      <div className="max-w-7xl 3xl:max-w-8xl 4xl:max-w-9xl 5xl:max-w-10xl mx-auto px-4 sm:px-6 lg:px-8 3xl:px-12 4xl:px-16 5xl:px-20 relative z-10">
        <div className="text-center mb-12 md:mb-16 3xl:mb-20 4xl:mb-24">
          ...
          <p className="text-lg xs:text-xl 3xl:text-2xl 4xl:text-3xl text-muted-foreground max-w-3xl 3xl:max-w-4xl 4xl:max-w-5xl mx-auto gsap-fade-up text-pretty leading-relaxed">
            My journey through AI research, cloud development, and software
            engineering that shaped my expertise in intelligent systems
          </p>
        </div>
              ...
              >
                <div className="timeline-dot absolute left-3 sm:left-4 md:left-1/2 transform md:-translate-x-1/2 w-8 h-8 sm:w-10 sm:h-10 3xl:w-12 3xl:h-12 4xl:w-14 4xl:h-14 bg-card border-2 sm:border-4 3xl:border-6 border-primary rounded-full z-10 flex items-center justify-center shadow-lg shadow-primary/20">
                  {React.cloneElement(exp.icon, {
                    className:
                      "w-4 h-4 sm:w-5 sm:h-5 3xl:w-6 3xl:h-6 4xl:w-7 4xl:h-7 text-primary",
                  })}
                </div>
                ...
                <div
                  className={`w-full md:w-1/2 ${
                    index % 2 === 0
                      ? "md:pr-6 lg:pr-8 3xl:pr-12 4xl:pr-16"
                      : "md:pl-6 lg:pl-8 3xl:pl-12 4xl:pl-16"
                  } ml-12 sm:ml-16 md:ml-0`}
                >
                  <Card className="timeline-card professional-shadow hover:shadow-xl hover:shadow-primary/5 transition-all duration-500 border-border/30 glass-morphism hover-lift">
                    <CardContent className="p-4 sm:p-6 3xl:p-8 4xl:p-10">
                      <div className="flex items-center justify-between mb-3">
                        <Badge
                          variant="outline"
                          className={`px-3 py-1 text-xs ${getTypeColor(
                            exp.type
                          )}`}
                        >
                      ...
                      <div className="flex flex-col xs:flex-row xs:items-center xs:space-x-2 text-xs sm:text-sm 3xl:text-base 4xl:text-lg text-muted-foreground mb-2 3xl:mb-4">
                        <div className="flex items-center space-x-2 mb-1 xs:mb-0">
                          <Calendar
                            size={14}
                            className="sm:w-4 sm:h-4 3xl:w-5 3xl:h-5 4xl:w-6 4xl:h-6"
                          />
