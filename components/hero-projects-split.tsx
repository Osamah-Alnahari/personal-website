...
export default function HeroProjectsSplit() {
  ...
  const handleFirstRowMouseEnter = () => {
    if (firstRowAnimation.current) {
      gsap.to(firstRowAnimation.current, {
        timeScale: 0,
        duration: 0.5,
        ease: "power2.out",
      });
  ...
  const handleSecondRowMouseEnter = () => {
    if (secondRowAnimation.current) {
      gsap.to(secondRowAnimation.current, {
        timeScale: 0,
        duration: 0.5,
        ease: "power2.out",
      });
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none hidden md:block">
        <Brain
          className="tech-icon absolute top-20 left-20 text-primary/30"
          size={32}
        />
        <Cloud
          className="tech-icon absolute top-40 right-32 text-accent/30"
          size={28}
        />
        <Zap
          className="tech-icon absolute bottom-40 left-32 text-secondary/30"
          size={24}
        />
        <Brain
          className="tech-icon absolute bottom-20 right-20 text-primary/20"
          size={36}
        />
        <Cloud
          className="tech-icon absolute top-60 left-1/2 text-accent/20"
          size={30}
        />
      </div>
          ...
          >
            <div className="space-y-4 lg:space-y-6">
              ...
              <p className="text-sm xs:text-base sm:text-lg md:text-lg 3xl:text-xl 4xl:text-xl text-muted-foreground max-w-lg lg:max-w-lg 3xl:max-w-2xl 4xl:max-w-3xl text-pretty leading-relaxed mx-auto lg:mx-0">
                Architecting intelligent systems that leverage cutting-edge AI
                models, cloud infrastructure, and scalable full-stack solutions
              </p>
            </div>
          ...
          >
            <div className="space-y-4 sm:space-y-6 md:space-y-8 3xl:space-y-12 4xl:space-y-16 w-auto self-stretch bg-transparent rounded-2xl sm:rounded-3xl p-4 border-0 shadow-none hover:shadow-none hover:border-transparent transition-all duration-500">
              ...
              >
                <div
                  ref={firstRowRef}
                  className="flex gap-3 sm:gap-4 md:gap-6 lg:gap-8 3xl:gap-12 4xl:gap-16 w-full"
                >
              ...
              >
                <div
                  ref={secondRowRef}
                  className="flex gap-3 sm:gap-4 md:gap-6 lg:gap-8 w-full"
                >
                  {[
                    ...allProjects.slice().reverse(),
                    ...allProjects.slice().reverse(),
                  ].map((project, index) => (
                    <div
                      key={`row2-${index}`}
